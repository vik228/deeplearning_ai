...
class NaiveBayes(object):
    ...
    def __init__(self, texts, sentiments):
        ...
        self.log_prior = 0
        self.log_likelihood = {}
        self.positives = [word for (word, sentiment) in self.freqs.keys() if sentiment == 1]
        self.negatives = [word for (word, sentiment) in self.freqs.keys() if sentiment == 0]
    ...
    def likelihood(self):
        for word in self.all_words:
            p_w_pos = (self.freqs.get((word, 1), 0) + 1)/ (len(self.positives) + len(self.all_words))
            p_w_neg = (self.freqs.get((word, 0), 1) + 1)/ (len(self.negatives) + len(self.all_words))
