...
class DecisionTree:
    ...
    def get_potential_splits_for_contineous_data(self):
        ...
        for col_ndex in range(cols - 1):
            ...
            for index in range(1, len(unique_values)):
                potential_splits[col_ndex].append(
                    (unique_values[index - 1] + unique_values[index]) / 2)
    ...
    def calculate_overall_entropy(self, split_column, split_value):
        data_below, data_above = self.split_contineous_data(
        ...
        self._overall_entropy = p_data_below * DecisionTree.entropy(
            data_below) + p_data_above * DecisionTree.entropy(data_above)
    ...
    def determine_best_split(self):
        ...
        potential_splits = self.get_potential_splits_for_contineous_data(
            self.X)
    ...
    def fit(self):
        if self.is_pure_split():
            ...
            data_below, data_above = self.split_contineous_data(
                best_split_col, best_split_value)
